...
nd <- data.frame(
  log_depth = seq(min(data$log_depth),
    max(data$log_depth),
    length.out = 30
  ),
  year = max(data$year) # a chosen year
...
plot_map(p2, "exp(est2)*1000000") +
  ...
  labs(fill = bquote(atop("Biomass per"~km^2,"given encounter")), 
...
plot_map(p, "est*1000000") +
  ...
  labs(fill = expression("Biomass per"~km^2), colour = expression("Biomass per"~km^2)) +
