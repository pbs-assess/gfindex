...
The report was run at `lubridate::now()` using sdmTMB version `r packageVersion("sdmTMB")`, TMB version `r packageVersion("TMB")`, and INLA version `r packageVersion("INLA")`.
...
- `r if (isFALSE(params$anisotropy)) 'Use a penalized complexity (PC) prior on the Matérn covariance parameters such that P(range > 5km) = 0.95 and P(SD < 3) = 0.95.'`
...
```{r plot-index, eval=plot_index, out.width="50%", fig.width = 8, fig.cap="Standardized index. Shown are means and 95% CIs."}
...
i %>%
  ...
  knitr::kable(digits = c(0, 0, 0, 0, 2), caption = "Standardized index of abundance. Estimates are in units of tonnes.")
```
