...
data %>% arrange(-density) %>% plot_multiyear_survey_sets(., region)
...
plot_map(p2, "exp(est2)") +
  ...
  labs(fill = expression("Biomass per"~m^2~"\ngiven encounter"), colour = expression("Biomass per"~m^2~"\ngiven encounter")) +
...
plot_map(p, "est") +
  ...
  labs(fill = expression("Biomass per"~m^2), colour = expression("Biomass per"~m^2)) +
