...
ggplot(filter(data, density > 0), aes_string("X", "Y", size = "density")) +
  geom_point(data = filter(data, density == 0), pch = 4, size = 0.8, colour = "grey60") +
  geom_point(colour = col[3]) +
  scale_size_area() +
  # scale_colour_viridis_c(trans = "log10") +
  coord_fixed() +
  labs(colour = "Density (kg/m^2)", size = "Density (kg/m^2)") +
  facet_wrap(~year) +
  ggtitle("Raw data")
```
...
```{r mesh, fig.width=5.5, fig.height=5.5}
# standard settings
cutoff <- 10
spatiotemporal <- "iid"
...
if (update_model) {
  ...
  if (!s$all_ok && isTRUE(family$delta)) {
    ...
    m1 <- update(m, spatiotemporal = list("off", "iid"))
    s <- sanity(m)
  }
...
```{r plot-index, eval=plot_index}
f <- here(paste0("indices/i-", spp, "-", ssid_string, "-", covariate_type, ".rds"))
if (file.exists(f)) {
  i <- readRDS(f)
  g <- ggplot(i, aes(year, est / 1000)) +
    geom_line(alpha = 0.3, lty = 1, lwd = 0.5) +
...
```{r plot-predictions, eval=plot_index}
...
```{r plot-biomass, eval=plot_index}
