...
`r if (params$region == "SYN WCHG") "SYN WCHG raw data are plotted but are excluded from index standardization."`
...
plot_map(p2, "exp(est2)") +
  facet_wrap(~year) +
   scale_fill_viridis_c(
    ...
    limits = c(0, quantile(exp(p2$est2), 0.99, na.rm = TRUE))
  ) +
...
plot_map(p, "est") +
  ...
  scale_fill_viridis_c(
    trans = "sqrt", na.value = "yellow",
    limits = c(0, quantile(p$est, 0.99, na.rm = TRUE))
  ) +
