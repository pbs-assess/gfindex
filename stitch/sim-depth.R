...
set.seed(SEED * 889)
yrs <- rnorm(max(predictor_dat$year), mean = 1.2, sd = 0.1)
...
# xx <- seq(0, 1, length.out = 100);plot(xx, xx * 1.2 + 1.5 * xx^2)
...
g <- mutate(geo_index_rw, type = "RW", with_depth = "depth = TRUE") %>%
  ...
  bind_rows(mutate(geo_index_no_region, type = "IID", with_depth = "depth = FALSE")) %>%
  bind_rows(mutate(geo_index_rw_no_region, type = "RW", with_depth = "depth = FALSE")) %>%
