...
# priors <- sdmTMBpriors(
#   matern_s = pc_matern(range_gt = 0.25, sigma_lt = 0.5),
#   matern_st = pc_matern(range_gt = 0.25, sigma_lt = 0.4)
# )
...
fit_rw <- sdmTMB(
  ...
  silent = TRUE, mesh = mesh# , priors = priors
)
...
mult <- 1
mutate(geo_index_rw, type = "RW", with_region = "region = TRUE") %>%
  ...
  ggtitle("IID vs. RW model; with and without region",
    subtitle = "Dashed = true; dots/lines = estimated") +
